...
function load_machine_rows(frm) {
    ...
    return frappe.call({
        ...
        setTimeout(() => {
            ...
            frappe.show_alert({
                message: __('Reload complete. Added {0} new LDV(s). Existing captured rows were kept.', [added_count]),
                indicator: 'green'
            });
        }, 200);
    }).catch(err => {
...
const MULTIUSER_HANDLER_KEY =
    'engineering_ldv_checklist_register_multiuser_handlers_bound';
...
function should_load_machine_rows(frm) {
    ...
    if (frm.is_new() || rows.length === 0) {
        return true;
    }

    const has_real_machine_row = rows.some(row => {
        return normalize_text(row.fleet_no) ||
            normalize_machine_type(row.machine_type) ||
            normalize_text(row.item_name);
    });

    return !has_real_machine_row;
}
...
function export_checklist_rows(frm) {
    ...
    const safe_name = normalize_text(frm.doc.name || 'Engineering LDV Checklist Register')
        .replace(/[^\w\-]+/g, '_');
