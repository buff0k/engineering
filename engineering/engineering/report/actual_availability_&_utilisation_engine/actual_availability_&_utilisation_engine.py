...
def calculate_availability_values(row):
    ...
    #
    # ONE AVAILABLE HOURS FORMULA:
    #
    # Available Hours =
    # Required Hours - PBM Total Downtime
    #
    # The calculation is valid only when the physical hours
    # fit inside the Required Hours for that shift.
    #
    # For a normal 2x12 shift:
    #
    # Required Hours = 12
    #
    # If:
    #
    # Work Hours + PBM Total Downtime > 12
    #
    # then Available Hours and the A&U percentages remain blank.
    # ========================================================

    available_is_valid = (
        required_hours > 0
        and work_hours <= required_hours
        and pbm_total_downtime <= required_hours
        and (
            work_hours
            + pbm_total_downtime
            <= required_hours
    ...
    if available_is_valid:
        ...
        # Invalid shift.
        #
        # Do NOT create artificial Available Hours.
        available_hours = None
        availability_percentage = None
        utilisation_percentage = None

    # Keep the old internal utilisation field synchronized.
    #
    # It is no longer displayed as a separate column.
    # Both Availability and Utilisation now use the same
...
def validate_au_row(row):
    ...
    validation_limit_hours = required_hours
    ...
    if work_hours > validation_limit_hours:
        invalid_reasons.append(
            ...
            f"{validation_limit_hours:g}h required shift limit"
        )
    ...
    if (
        ...
        invalid_reasons.append(
            ...
            f"exceeds {validation_limit_hours:g}h required shift limit"
        )
...
def get_required_hours(
    ...
    shift_system = str(
        planning.get("shift_system") or ""
    ).strip().lower()

    # Actual Availability & Utilisation Engine:
    #
    # 2x12 operation:
    # Day   = 12 required hours
    # Night = 12 required hours
    # Both  = 24 required hours
    if shift_system == "2x12hour":
        return (
            12.0
    ...
    # Keep existing configured required hours
    # for other shift systems.
    field_map = {
        "Day": "shift_day_hours",
    ...
    def calculate_required_downtime(
        ...
        for au_row in candidate_rows:
            try:
                ...
                # Actual A&U report uses fixed physical 12-hour
                # Day/Night windows for PBM downtime:
                #
                # Day   06:00 -> 18:00
                # Night 18:00 -> 06:00 next day
                if au_row.shift == "Day":
                    shift_start = get_datetime(
                        f"{au_row.shift_date} 06:00:00"
                    )

                    shift_end = get_datetime(
                        f"{au_row.shift_date} 18:00:00"
                    )

                elif au_row.shift == "Night":
                    shift_start = get_datetime(
                        f"{au_row.shift_date} 18:00:00"
                    )

                    shift_end = get_datetime(
                        f"{add_days(au_row.shift_date, 1)} 06:00:00"
                    )
                ...
                if au_row.shift in ("Day", "Night"):
                    # Actual PBM downtime is the PBM overlap inside the
                    # physical 12-hour shift window.
                    #
                    # It is independent of:
                    # - Planned Downtime
                    # - Startup/Fatigue
                    # - Sunday planning exclusions
                    required_downtime_hours = min(
                        shift_overlap_hours,
                        12.0,
                    )

                elif required_hours <= 0:
                    if (
                        getdate(
                            au_row.shift_date
                        ).weekday() == 6
...
def get_invalid_au_pbm_records(
    ...
    if shift == "Day":
        shift_start = get_datetime(
            f"{shift_date_value} 06:00:00"
        )

        shift_end = get_datetime(
            f"{shift_date_value} 18:00:00"
        )
        ...
        shift_end = get_datetime(
            f"{add_days(shift_date_value, 1)} 06:00:00"
        )
